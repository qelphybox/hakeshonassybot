...
describe('renderMessage', () => {
  test('TODAY_MESSAGE_COUNT', () => {
    const statsArray = [{ name: stats.TODAY_MESSAGE_COUNT, data: [{username: 'test', count: 1}, {username: 'test2', count: 2}] }]
    expect(renderMessage(statsArray)).toBe(`Сообщений за последние 24 часа: @test (1), @test2 (2)`);
  });

  test('HOUR_MESSAGE_COUNT', () => {
    const statsArray = [{ name: stats.HOUR_MESSAGE_COUNT, data: [{username: 'test', count: 1}, {username: 'test2', count: 2}] }]
